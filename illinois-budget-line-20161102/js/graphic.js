...
var box24 = chartElement.append('g')
        .attr('class', 'annotation-box')
        .append('rect');
...
var boxHold = chartElement.append('g')
    ...
    chartElement.append('g')
        ...
        .append('text')
        .attr('x', function() {
                return xScale(d3.time.format('%m/%d/%Y').parse('8/10/2016'))+10
            })
            .attr('y', function(d) {
                return yScale(388.2) + 10
            })
            .text(function(d) {
                return "Teachers Retirement System payment"
            })
            .attr('class','aug10');
    ...
    chartElement.append('g')
        ...
        .append('text')
            .attr('x', function() {
                return xScale(d3.time.format('%m/%d/%Y').parse('8/19/2016'))-20
            })
            .attr('y', function(d) {
                return yScale(200) - 20
            })
            .text(function(d) {
                return "General State Aid to schools"
            })
            .attr('class','aug19');
    ...
    chartElement.append('g')
        ...
        .append('text')
            .attr('x', function() {
                if (isMobile) {
                    return xScale(d3.time.format('%m/%d/%Y').parse('8/24/2016'))
                } else {
                    return xScale(d3.time.format('%m/%d/%Y').parse('8/24/2016')) + 20
                }
            })
            .attr('y', function(d) {
                return yScale(134.8)+20
            })
            // .text(function(d) {
            //     return "State University Retirement System payment"
            // })
            .attr('class','aug24')
                .append("tspan").text('State University')
                .append("tspan")
                    .attr('x', function() {
                        if (isMobile) {
                            return xScale(d3.time.format('%m/%d/%Y').parse('8/24/2016'))
                    ...
                    .attr("dy", "1.3em").text('Retirement System')
                .append("tspan")
                    .attr('x', function() {
                        if (isMobile) {
                            return xScale(d3.time.format('%m/%d/%Y').parse('8/24/2016'))
    ...
    chartElement.append('g')
        ...
        .append('text')
            .attr('x', function() {
                if (isMobile) {
                    return xScale(d3.time.format('%m/%d/%Y').parse('8/4/2016'))
                } else {
                    return xScale(d3.time.format('%m/%d/%Y').parse('8/4/2016')) + 20
                }
            })
            .attr('y', function(d) {
                return yScale(134.8) -20
            ...
            .text(function(d) {
                return "Payments held"
            })
            .attr('class','hold');

   box10bounds = d3.select(".aug10").node().getBBox()
   box19bounds = d3.select(".aug19").node().getBBox()
   box24bounds = d3.select(".aug24").node().getBBox()
   boxHoldBounds = d3.select(".hold").node().getBBox()
    ...
    box24
        .attr('x', function() {
                return box24bounds.x -5;
            })
            .attr('y', function(d) {
                return box24bounds.y -5;
            })
            .attr('width', function(d) {
                return box24bounds.width +10;
            })
            .attr('height', function(d) {
                return box24bounds.height +10;
            })
    ...
    chartElement.append("line")
        ...
        .attr("x2", xScale(d3.time.format('%m/%d/%Y').parse('8/07/2016'))) 
        .attr("y2", yScale(50))
    ...
    chartElement.append("line")
        ...
        .attr("x1", boxHoldBounds.x+(boxHoldBounds.width/2))
        .attr("y1", boxHoldBounds.y+(boxHoldBounds.height))
        .attr("x2", xScale(d3.time.format('%m/%d/%Y').parse('8/14/2016'))) 
        .attr("y2", yScale(50))
        .attr("marker-end", "url(#arrow)");
    ...
    chartElement.append("line")
        ...
        .attr("x1", boxHoldBounds.x+(boxHoldBounds.width/2))
        .attr("y1", boxHoldBounds.y+(boxHoldBounds.height))
        .attr("x2", xScale(d3.time.format('%m/%d/%Y').parse('8/22/2016'))) 
        .attr("y2", yScale(40))
        .attr("marker-end", "url(#arrow)");
    ...
    // chartElement.append("line")
    //     .attr('class','arrowline')
    //     .attr("x1", box10bounds.x)
    //     .attr("y1", box10bounds.y+(box10bounds.height/2))
    //     .attr("x2", xScale(d3.time.format('%m/%d/%Y').parse('8/10/2016'))) 
    //     .attr("y2", yScale(375.4))
    //     .attr("marker-end", "url(#arrow)");
